...
class LettersCmds(commands.Cog):
	...
	async def role(self, ctx, *, role: discord.Role):
		...
		embed.add_field(
			name="Hex color code", value=f"**{role.color}**"
	...
	async def whitenoise(self, ctx, length: int = 5, samplingrate: int = 10000):
		...
		bytes = numpy.array(
			ran, dtype=numpy.int8
		)  # we'll feed this to our wav file
	...
	async def imageinfo(self, ctx):
		...
		emb = discord.Embed(title=f"Image information for {img.filename}")
		...
		emb.color = discord.Colour.from_rgb(
			comcol[1][0], comcol[1][1], comcol[1][2]
		)
