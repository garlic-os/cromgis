...
def reupload_to_imgur(video_url: str) -> str:
	params = {"client_id": os.environ["IMGUR_CLIENT_ID"]}
	payload = {
		"video": video_url,
		"type": "URL",
		"disable_audio": "1",
	...
	resp_upload = requests.post(
		"https://api.imgur.com/3/image", params=params, json=payload
	)
	...
	while len(done) == 0:
		...
		resp_album = requests.get(
			"https://imgur.com/upload/poll", params=params
		)
...
class LoopCommand(commands.Cog):
	...
	@commands.command()
	async def loop(
		self, ctx: commands.Context, *, video_url: str = None
	) -> None:
		"""Loop an MP4"""
		await ctx.typing()
		...
		link = await self.bot.loop.run_in_executor(
			None, reupload_to_imgur, video_url
		)
