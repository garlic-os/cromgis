...
def sort_pixels_by_color(img: Image.Image) -> Image.Image:
	...
	"""
	# 2D array of image's pixels
	# shape: (..., ..., 4)
	raster: npt.NDArray[np.uint8] = plt.pil_to_array(img)

	# Keep the raster's shape in a separate variable because it is going to
	# outlive the rest of the object
	raster_shape = raster.shape

	# assert len(raster_shape) == 3
	# assert raster_shape[2] == 4

	# 1D array of raster's pixels + extra information per pixel. So pixels can
	# be sorted as a 1D array
	# shape: (..., 6)
	pixels = np.ndarray(
		(raster_shape[0] * raster_shape[1]),
		dtype=[
			("r", np.uint8),
			("g", np.uint8),
			("b", np.uint8),
	...
	for i in range(raster_shape[0]):
		for j in range(raster_shape[1]):
			...
			pixels[i * raster_shape[1] + j][0] = r
			pixels[i * raster_shape[1] + j][1] = g
			pixels[i * raster_shape[1] + j][2] = b
			pixels[i * raster_shape[1] + j][3] = a
			pixels[i * raster_shape[1] + j][4] = color_group.value
			pixels[i * raster_shape[1] + j][5] = luminosity

	# Done with the old raster; throw it away and create a new one that we can
	# write to
	raster = np.ndarray(shape=raster_shape, dtype=np.uint8)

	# Sort pixels by color group first, then by luminosity
	# TODO: optimization: use `indices` to copy `pixels` into `raster` directly
	indices = np.lexsort((pixels["luminosity"], pixels["color_group"]))
	pixels = pixels[indices]  # the big object in question

	# Put sorted pixels back into the raster.
	for i in range(raster_shape[0]):
		for j in range(raster_shape[1]):
			for k in range(4):
				raster[i, j, k] = pixels[i * raster_shape[1] + j][k]
