...
v_titles = [
	...
	"Ooh!",
	"Exciting!",
	"The vending machine bestows you an offering.",
	"Thrilling!",
	"Amazing!",
	"Hmmm...",
	"What?",
...
def uwuizeText(text: str):
	...
	text = re.sub(
		"([\s!.～~])@@󺘆@@@", "... " + random.choice(uwuSounds) + "~", text
	)
	...
	if q == "n":
		text = re.sub(
			"([\s!.～~])````󺘆@``",
			"~ *nyuzzewes u" + random.choice(uwuNuzzles),
			text,
		)
		text = re.sub("@@󺘆@@@", random.choice(uwuSounds), text)
		text = re.sub(
			"````󺘆@``", "... *nyuzzewes u" + random.choice(uwuNuzzles), text
		)
	elif q == "s":
		text = re.sub(
			"([\s!.～~])````󺘆@``",
			"~ *snyiffs u" + random.choice(uwuSniffs),
			text,
		)
		text = re.sub("@@󺘆@@@", random.choice(uwuSounds), text)
		text = re.sub(
			"````󺘆@``", "... *snyiffs u" + random.choice(uwuSniffs), text
		)
	else:
		text = re.sub(
			"([\s!.～~])````󺘆@``", "~ *pwuwws" + random.choice(uwuNuzzles), text
		)
		text = re.sub("@@󺘆@@@", random.choice(uwuSounds), text)
		text = re.sub(
			"````󺘆@``", "... *pwuwws" + random.choice(uwuNuzzles), text
		)
	return text
...
def bakaText(text: str):
	...
	for i in text2:
		if (random.randint(0, 2) == 1) or (len(text2) < 3):
			ret += re.sub(
				r"(?P<a>\b[a-zA-Z])",
				r"\g<a>-" * random.randint(1, 2) + r"\g<a>",
				i,
			)
		else:
			ret += i
